---
name: map-learn
description: Close reviewed agent-collab findings into provider-neutral MAP learning records for Codex, Grok, and Claude after exact profile and provenance verification.
---
...
- Encode the candidate as canonical JSON followed by one newline. Its `consumerScopes` must be exactly `codex`, `grok`, and `claude`; control IDs must be sorted.
...
This skill does not authorize provider calls, MCP changes, `mapify` installation or upgrade, Git operations, publication, or mutable Grok/Claude execution.
